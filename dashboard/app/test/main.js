...
var steps = {
	...
	ValidateLaunchedApp: function (test) {
		return waitForUrl(/\/apps\/[^\/]+$/, function () {
			var editEnvLinkSelector = 'a[href$="/env"]';
			test.assertExists(editEnvLinkSelector, 'Edit env link exists');
			casper.click(editEnvLinkSelector);
			return waitForSelector('#secondary .edit-env input', function () {});
		}).then(function () {
			...
			test.assertExists(testEnvKeySelector, 'Env key persisted');
			test.assertExists(testEnvValueSelector, 'Env value persisted');
		});
	},
...
casper.test.begin('Dashboard integration test', function (test) {
	...
	casper.then(function () {
		...
		return steps.ValidateLaunchedApp(test);
	});
